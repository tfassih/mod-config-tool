...
maps = [{"name": "Alborz Mountains", "mapID": "XP3_Alborz", "modes": [modes[2], modes[3], modes[9], modes[11], modes[14], modes[15]]},
        {"name": "Armored Shield", "mapID": "XP3_Shield", "modes": [modes[2], modes[3], modes[9], modes[11], modes[14], modes[15]]},
        ...
        {"name": "Caspian Border", "mapID": "MP_007", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Damavand Peak", "mapID": "MP_013", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        ...
        {"name": "Grand Bazaar", "mapID": "MP_001", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Gulf of Oman", "mapID": "XP1_002", "modes": [modes[2], modes[3], modes[4], modes[6], modes[9], modes[11], modes[14], modes[15]]},
        {"name": "Kharg Island", "mapID": "MP_018", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Kiasar Railroad", "mapID": "XP5_003", "modes": [modes[1], modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Markaz Monolith", "mapID": "XP4_FD", "modes": [modes[4], modes[5], modes[8], modes[9], modes[10], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Nebandan Flats", "mapID": "XP5_002", "modes": [modes[1], modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Noshahr Canals", "mapID": "MP_017", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Operation 925", "mapID": "XP2_Office", "modes": [modes[7], modes[8], modes[11], modes[15]]},
        {"name": "Operation Firestorm", "mapID": "MP_012", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Operation Metro", "mapID": "MP_Subway", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Operation Riverside", "mapID": "XP5_001", "modes": [modes[1], modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Sabalan Pipeline", "mapID": "XP5_004", "modes": [modes[1], modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Scrapmetal", "mapID": "XP2_Factory", "modes": [modes[7], modes[8], modes[11], modes[15]]},
        {"name": "Seine Crossing", "mapID": "MP_011", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Sharqi Peninsula", "mapID": "XP1_003", "modes": [modes[4], modes[5], modes[6], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Talah Market", "mapID": "XP4_Rubble", "modes": [modes[4], modes[5], modes[8], modes[9], modes[10], modes[11], modes[12], modes[14], modes[15]]},
        {"name": "Tehran Highway", "mapID": "MP_003", "modes": [modes[2], modes[3], modes[9], modes[11], modes[12], modes[14], modes[15]]},
        ]
...
class BF3FunBotConfigFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.new_vu_server_config_button = ctk.CTkButton(self,
                                                     text="New VU Server Config",
                                                     text_color="#FFFFFF",
                                                     height=30,
                                                     width=225,
                                                     font=("Segoe UI", 14),
                                                     command=self.init_new_vu_server_config,
                                                     state="enabled",
                                                      anchor="bottom"
                                                     )
        self.new_vu_server_config_button.grid(row=0,
                                          column=0,
                                          padx=25,
                                          pady=65,
                                          sticky="esw",
        ...
        self.new_vu_server_config_window = None

    def init_new_vu_server_config(self):
        if self.new_vu_server_config_window is None or not self.new_vu_server_config_window.winfo_exists():
            self.new_vu_server_config_window = NewBF3VuServerConfigWindow(self)
            self.new_vu_server_config_window.grab_set()
            self.new_vu_server_config_window.focus_set()
        else:
...
class NewBF3VuServerConfigWindow(ctk.CTkToplevel):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
        self.title("New BF3 VU Server Config")
        ...
        # Load Config JSON
        with open("bf3-reference-settings/settings-definition.json", "r") as file:
            self.config_data = json.load(file)["SettingsDefinition"]
    ...
    def generate_config_elements(self):
        ...
        for key, value in self.config_data.items():
            ...
            if isinstance(value, bool):  # Boolean fields as dropdowns
                ...
                option_menu.grid(row=row, column=column * 2 + 1, padx=10, pady=5,
                                 sticky="e")  # Use odd column for inputs
    ...
    def export_to_files(self):
        ...
        export_dir = "user-generated/vu config"
        ...
        json_file_path = os.path.join(export_dir, "settings-definition.json")
        lua_file_path = os.path.join(lua_export_dir, "settings-definition.lua")

        try:
            # Save as JSON
            with open(json_file_path, "w", encoding="utf-8") as json_file:
                json.dump({"SettingsDefinition": self.config_data}, json_file, indent=4)
